...
export default class MarkerItem extends React.Component {
    ...
    editMarker(e) {
        e.preventDefault();
        this.props.dispatch(displayCreateEditModal({display: true, markerInfo: this.props.marker}));
    }
    ...
    render() {
        return(
            <div className="card card-body display-inline">
                ...
                <button className="button-cls ml0" onClick={this.editMarker.bind(this)}>Edit</button>
                <button className="button-cls" onClick={this.deleteMarker.bind(this)}>Delete</button>
            </div>
        )
    }
}